...
import { getActiveBridgeUrl } from "../src/instance-discovery.js";
import {
  getRequestAgentId,
...
test("parallel request contexts keep agent and port routing isolated", async () => {
  const [first, second] = await Promise.all([
    runWithRequestContext({ agentId: "agent-a", portOverride: 7891 }, async () => {
      await delay(20);
      return { agentId: getRequestAgentId(), bridgeUrl: getActiveBridgeUrl() };
    }),
    runWithRequestContext({ agentId: "agent-b", portOverride: 7892 }, async () => {
      await delay(5);
      return { agentId: getRequestAgentId(), bridgeUrl: getActiveBridgeUrl() };
    }),
  ]);

  assert.deepEqual(first, { agentId: "agent-a", bridgeUrl: "http://127.0.0.1:7891" });
  assert.deepEqual(second, { agentId: "agent-b", bridgeUrl: "http://127.0.0.1:7892" });
});
