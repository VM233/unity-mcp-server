...
生成时间：2026-07-29T22:46:35.147Z

## 结论

基线有 58 个 Node 工具和 127 个插件第一类工具，原始合计 185 项；按名称去重后为 174 项，并存在 11 个跨层同名碰撞。修复后默认面为 63 项，其中插件发布策略固定为 33 条路由。

174 个基线唯一工具的处理结果：默认保留 60、懒加载 87、合并 4、内部化 5、三段式 18。

当前默认面新增的规范入口：`unity_build`、`unity_search_scene`、`unity_scene_instantiate_prefab`。

## 整体框架审查与已实施修复

| 领域 | 发现的问题 | 已实施修复 | 当前判定 |
|---|---|---|---|
| 工具面 | 185 个原始条目会挤占上下文，且有 11 个同名跨层碰撞 | 建立 63 项默认面、33 路由发布策略和 lazy 兼容层 | 已修复 |
...
- 插件元数据：33 个内建第一类路由，质量问题 0。
- Node：63 个默认工具，39/39 自动测试通过。
- npm：生产依赖漏洞 0。
- 包测试：第一轮 180 项发现 2 个审查期回归；修复后最终全量 181/181 通过，manifest 精确恢复。
